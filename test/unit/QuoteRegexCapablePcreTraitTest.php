...
use Dhii\Regex\QuoteRegexCapablePcreTrait as TestSubject;
use Xpmock\TestCase;
 ...
 * @since [*next-version*]
 */
class QuoteRegexCapablePcreTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Regex\QuoteRegexCapablePcreTrait';
    ...
    public function testQuoteRegex()
    {
        $text = '\w';
        $delim = '/';
        $string = vsprintf('%1$s%2$s%1$s', [$delim, $text]);
        ...
        $subject->expects($this->exactly(2))
            ...
            ->withConsecutive([$string], [$delim])
            ->will($this->returnArgument(0));

        $result = $_subject->_quoteRegex($string, $delim);
        $this->assertEquals(preg_quote($string, $delim), $result, 'Quoting produced wrong result');
    }
}
